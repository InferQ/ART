...
// We are importing from source since we are in the monorepo context and build artifacts might be tricky with deep imports
// In a real app, these would be exported from the main package index
import { AgentFactory } from 'art-framework';
import { PESAgent } from 'art-framework';
import { IndexedDBStorageAdapter } from 'art-framework';
import { GeminiAdapter } from 'art-framework';
import { UISystem } from 'art-framework';
import { AgentProps, TodoItemStatus } from 'art-framework';
...
const App = () => {
  ...
  const [agent, setAgent] = useState<PESAgent | null>(null);
  const [threadId, setThreadId] = useState(uuidv4());
  const [todoList, setTodoList] = useState<any[]>([]);
  const [currentThought, setCurrentThought] = useState<string>('');
  const [intent, setIntent] = useState<string>('');
  const [uiSystem, setUiSystem] = useState<UISystem | null>(null); // State to hold UISystem
  ...
  const initAgent = async () => {
    ...
    try {
      ...
      const newAgent = await factory.createAgent({
        ...
        reasoningAdapter: gemini,
        tools: []
      });

      // Hack to access internal dependencies if not exposed.
      // Ideally, the factory or agent should expose the UI system or we subscribe differently.
      // The `PESAgent` instance has `deps`. Let's assume for this example we can cast to any.
      const agentAny = newAgent as any;
      if (agentAny.deps && agentAny.deps.uiSystem) {
          const uiSys = agentAny.deps.uiSystem as UISystem;
          setUiSystem(uiSys);

          // Subscribe to observations (Plan, Intent, etc.)
          uiSys.getObservationSocket().subscribe({
              next: (obs) => {
          ...
          // Subscribe to LLM Stream (Thoughts)
          uiSys.getLLMStreamSocket().subscribe({
             next: (event) => {
                 if (event.type === 'TOKEN') {
                    // Accumulate tokens. If tokenType indicates thought, append to thought.
                    // If regular response, we might not want to show it here if we wait for final.
                    // But for thinking...
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await agent.process(props);
